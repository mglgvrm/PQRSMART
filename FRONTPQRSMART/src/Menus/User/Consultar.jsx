...
const Consultar = () => {
    ...
    try {
      ...
      if (usuario) {
        const filteredData = response.data.filter(
          (item) => item.user && item.user.user === usuario,
        );
  ...
  const paginacion = filtered.slice(
  ...
  return (
    <div className="consultarPqrs">
      ...
      <div className="cuerpoconsultarPqrs">
        <div className="tabla-usuario">
          ...
          <label className="nota">
            Nota: Con La X Puede Cancelar Su Solicitud
          </label>
                  ...
                  <tr key={index}>
                    ...
                    <td>
                      <span className="span-descargar">
                        {pqrs.archivo ? (
                          <a
                            href={`https://pqrsmartback-production-86b1.up.railway.app/api/request/download/${encodeURIComponent(pqrs.archivo.split("\\").pop().split("/").pop())}`}
                            download
                            target="_blank"
                    ...
                    <td>
                      <span className="span-descargar">
                        {pqrs.archivoAnswer ? (
                          <a
                            href={`https://pqrsmartback-production-86b1.up.railway.app/api/request/download/${encodeURIComponent(pqrs.archivoAnswer.split("\\").pop().split("/").pop())}`}
                            download
                            target="_blank"
                            rel="noopener noreferrer"
                          >
                    ...
                    <td>
                      <span className="span-descargar">
                        {pqrs.evidenceAnswer ? (
                          <a
                            href={`https://pqrsmartback-production-86b1.up.railway.app/api/request/download/${encodeURIComponent(pqrs.evidenceAnswer.split("\\").pop().split("/").pop())}`}
                            download
                            target="_blank"
                            rel="noopener noreferrer"
                          >
          ...
          <div className="paginacion">
            ...
            <button
              onClick={() =>
                setCurrentPage((prev) => Math.min(prev + 1, totalPaginas - 1))
              }
      ...
      {show && (
        <VerPqrs isOpen={show} onClose={() => setShow(false)} pqrs={pqrs} />
      )}
